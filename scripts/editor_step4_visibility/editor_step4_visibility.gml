//case "editor_step4_visibility"
function editor_step4_visibility()
{
	//menus						
	...
	ds_list_add(clickable_list,xxx	+	xref*game_struct.button_size	+ 2*game_struct.button_size);//x2
	ds_list_add(clickable_list,yyy	+	yref*game_struct.button_size	+ 0.5*game_struct.button_size);//y2	
	ds_list_add(clickable_list,"Population");//txt
	ds_list_add(clickable_list,"_128x32_background");//picture
	ds_list_add(clickable_list,"left click \n to set population");//txt mouseover rclick in case of "_"
	ds_list_add(clickable_list,"exe_start_editor_step5");	//execution on click	
	ds_list_add(clickable_list,"_");	//execution on right click	
	...
	if string_length(_string_paint) > 1	{_string_paint = "Facility - visibility" + "\n" + string_delete(_string_paint, 1, _delete_string);}
	else	{_string_paint = "Facility - visibility";};
	ds_list_add(clickable_list,xxx	+	xref*game_struct.button_size);//x1
	ds_list_add(clickable_list,yyy	+	yref*game_struct.button_size);//y1
	ds_list_add(clickable_list,xxx	+	xref*game_struct.button_size	+ 4*game_struct.button_size);//x2
	ds_list_add(clickable_list,yyy	+	yref*game_struct.button_size	+game_struct.button_size);//y2	
	...
	for(var _hh = 0; _hh < array_length(sides_array); _hh++)
		{
			...
			ds_list_add(clickable_list,"exe_paint_visibility_");	//execution on click	
			ds_list_add(clickable_list,"_");	//execution on right click
			xref+=2;
			if xref > 3	{xref = 0; yref += 0.5;};
		}
	//or editor
	var _side_id =  "editor";
	ds_list_add(clickable_list,xxx	+	xref*game_struct.button_size);//x1
	...
	var _string_paint = string(mapeditor.terrain_paint_selected);
	var _delete_string = string_length("paint_visibility_");
	if string_length(_string_paint) > 1	{_string_paint = string_delete(_string_paint, 1, _delete_string);}
	if string_length(_string_paint) > 1
		{
			for(var _hh = 0; _hh < array_length(sides_array); _hh++)
				{
					var _side_id =  sides_array[@ _hh][$ "side_name"];
					if string_starts_with(_string_paint,_side_id)
						{
							var _avilable_utylity_array = sides_array[@ _hh][$ "ruins"];
							for(var _ii = 0; _ii < array_length(_avilable_utylity_array); _ii++)
								{
									var _this_utylity = sides_array[@ _hh][$ "ruins"][@ _ii];
									...
									ds_list_add(clickable_list,"");//txt
									ds_list_add(clickable_list,string(_this_utylity) );//picture
									ds_list_add(clickable_list,_this_utylity);//txt mouseover rclick in case of "_"
									ds_list_add(clickable_list,"exe_paint_ruins_choosen_");	//execution on click	
									ds_list_add(clickable_list,"_");	//execution on right click
									xref++;
