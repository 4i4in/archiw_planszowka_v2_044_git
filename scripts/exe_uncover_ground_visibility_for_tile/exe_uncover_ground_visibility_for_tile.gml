function exe_uncover_ground_visibility_for_tile(_ii,_xx,_yy,_val)
{
	...
	var _tile_worth_of_update = 0
		{
			...
			if _fog_ow_war[1] < _val	{_tile_worth_of_update = 1;};
		}
	if	_tile_worth_of_update > 0
		{
			array_set(_fog_ow_war,0,game_struct.current_turn);
			array_set(_fog_ow_war,1,_val);	
			
			//is terrain ground or not
			var _terrain_is_ground = string_char_at(_local_terrain,1);
			switch(_terrain_is_ground)
				{
					case "z" : _terrain_is_ground = 0; break;
					case "d" : _terrain_is_ground = 0; break;
				}
			if _terrain_is_ground != 0	{	_terrain_is_ground = 1;	};
			...
			if _local_tile_operational != _local_tile_gameboard
				{
							...
							for(var _ee = 0; _ee < array_length(_str_units_groups); _ee++)
								{
									...
									if _group_control == _this_side
									...
									else
										{
																			...
																			for(var _curent_buff = 0;_curent_buff < array_length(_buffs_array);_curent_buff+=2)
																				{
																					...
																					if string_starts_with(_buffs_array[_curent_buff],"overwater")
																						{
																							_hidden = 1;
																						}
																					if _hidden = 1
											...
											if _specific_visibility_array[0] + game_struct.visibility_level_struct[$ "vis_recognize_units_group"] < _val + _curious + _spy
												{
													_group_recognized = 1;
											...
											if _group_recognized > 0
												{
													for(var _kk = 1; _kk < array_length(_specific_visibility_array); _kk++)
														{
															if _specific_visibility_array[_kk] + game_struct.visibility_level_struct[$ "vis_recognize_unit_specyfic"] < _val	+ _spy
																{
											...
											else	//unrecognized group
												{
													for(var _kk = 1; _kk < array_length(_specific_visibility_array); _kk++)
														{
															if _specific_visibility_array[_kk] + game_struct.visibility_level_struct[$ "vis_recognize_units_number"] < _val + _inspector
																{
																	var _unit_found = return_unknown_unit();
																	array_push(_temp_group,_unit_found);
																}
											...
											var _you_see_shit = 0;
											if array_length(_temp_group) < 2	&&	_terrain_is_ground = 1
												{
													var _unit_found = return_unknown_unit();
													array_push(_temp_group,_unit_found);
												}
											if array_length(_temp_group) < 2	&&	_terrain_is_ground = 0
												{
													_you_see_shit = 1;
												}
											if _you_see_shit = 0
					...
					//ground
					if _terrain_is_ground = 1
						{
							//show vis_tile_resources
							if _val > game_struct.visibility_level_struct[$ "vis_tile_resources"]
								{
									_local_tile_operational[$ "tile_resources"] = _local_tile_gameboard[$ "tile_resources"];
								}
							//show vis_tile_props
							if _val > game_struct.visibility_level_struct[$ "vis_tile_props"]
								{
									_local_tile_operational[$ "tile_props"] = _local_tile_gameboard[$ "tile_props"];
								}
					...
					if _terrain_is_ground = 1
						{
							...
							if _val + _curious > game_struct.visibility_level_struct[$ "vis_granted"]
								{
									//terrain
									_local_tile_operational[$ "tile_terrain"][0] = _local_tile_gameboard[$ "tile_terrain"][0];
									//setings	
